package integracion.wordseedexporter.components;
...
/**
 * Esta clase es casi una copia exacta de PDFViewSkin (clase de PDFViewFX), con
 * la excepción de los comentarios y textos originales del inglés traducidos al
 * español.
 * 
 * Esto es debido a los métodos y objetos privados que la clase posee, por lo
...
public class PDFViewSkinES extends PDFViewSkin {
	...
	// Access to PDF document must be single threaded (see Apache PdfBox website
	// FAQs)
	...
	public PDFViewSkinES(PDFView view) {
		super(view);
		...
		searchResultListView.visibleProperty()
				.bind(Bindings.isNotEmpty(pageSearchResults).and(view.showSearchResultsProperty()));
		searchResultListView.managedProperty()
				.bind(Bindings.isNotEmpty(pageSearchResults).and(view.showSearchResultsProperty()));
		...
		view.selectedSearchResultProperty().addListener(it -> {
			...
			if (result != null) {
				pageSearchResults.stream().filter(r -> r.getPageNumber() == result.getPageNumber()).findFirst()
						.ifPresent(r -> {
		...
		view.pageProperty().addListener(it -> {
			...
			if (!pageSearchResults.isEmpty()) {
				/*
				 * We have page search results and the current page changes. Let's make sure we
				 * select a matching page result, if one exits.
				 */
				pageSearchResults.stream().filter(result -> result.getPageNumber() == view.getPage()).findFirst()
	...
	private <T> void maybeScrollTo(ListView<T> listView, T item) {
		/*
		 * We want to make sure that the selected result will be visible within the list
		 * view, but we do not want to scroll every time the selected search result
		 * changes. We really only want to perform a scrolling if the newly selected
		 * search result is not within the currently visible rows of the list view.
		 */
		...
		if (virtualFlow != null) {
			...
			if (firstVisibleCell != null && lastVisibleCell != null) {
				 ...
				 * currently visible cells doesn't seem to work perfectly. Also, if only a
				 * fraction of a cell is visible then it requires scrolling, too.
				 */
	...
	private ToolBar createToolBar(PDFView pdfView) {
		...
		showAll.setTooltip(new Tooltip("Mostrar todos / Toda la página"));
		...
		totalPages.setTooltip(new Tooltip("Número total de páginas"));
		...
		rotateLeft.setTooltip(new Tooltip("Rotar página hacia la izquierda"));
		...
		rotateRight.setTooltip(new Tooltip("Rotar página hacia la derecha"));
		...
		Tooltip.install(searchClearIcon, new Tooltip("Borrar búsqueda de texto"));

		CustomTextField searchField = new CustomTextField();
		searchField.setText("Buscar texto");
		...
		searchField.visibleProperty().bind(Bindings.createBooleanBinding(
				() -> pdfView.getDocument() instanceof SearchableDocument, pdfView.documentProperty()));

		Region spacer = new Region();
		HBox.setHgrow(spacer, Priority.ALWAYS);

		// toolbar
		return new ToolBar(showAll, new Separator(Orientation.VERTICAL), zoomLabel, zoomSlider,
				new Separator(Orientation.VERTICAL), pageControl, new Separator(Orientation.VERTICAL), rotateLeft,
				rotateRight, spacer, searchField);
	}
	...
	private HBox createSearchNavigator() {
		...
		searchLabel.textProperty()
				.bind(Bindings
						.createObjectBinding(
								() -> "Encontradas " + view.getSearchResults().size() + " ocurrencias en "
										+ pageSearchResults.size() + " páginas",
								view.getSearchResults(), pageSearchResults));
		...
		previousResultButton.setTooltip(new Tooltip("Ir al resultado anterior de búsqueda"));
		...
		nextResultButton.setTooltip(new Tooltip("Ir al siguiente resultado de búsqueda"));
	...
	class MainAreaScrollPane extends ScrollPane {
		...
		public MainAreaScrollPane() {
			...
			wrapper = new StackPane() {
				...
				protected void layoutChildren() {
					...
					if (result != null) {
						...
						final Rectangle2D marker = result
								.getScaledMarker(pdfView.getPageScale() * pdfView.getZoomFactor());
			...
			mainAreaRenderService.scaleProperty()
					.bind(pdfView.pageScaleProperty().multiply(pdfView.zoomFactorProperty()));
	...
	private class RenderTask extends Task<Image> {
		...
		private void highlightSearchResults(int pageNumber, float scale, BufferedImage bufferedImage) {
			final List<SearchResult> searchResults = getSkinnable().getSearchResults().stream()
					.filter(result -> result.getPageNumber() == pageNumber).collect(Collectors.toList());
			...
			if (!searchResults.isEmpty()) {
				...
				graphics.setColor(new java.awt.Color((int) (255 * searchResultColor.getRed()),
						(int) (255 * searchResultColor.getGreen()), (int) (255 * searchResultColor.getBlue())));

				searchResults.forEach(result -> {
					Rectangle2D highlightMarker = result.getScaledMarker(scale);
					graphics.fillRect((int) highlightMarker.getMinX(), (int) highlightMarker.getMinY(),
							(int) highlightMarker.getWidth(), (int) highlightMarker.getHeight());
				});
			}
		}
	}
		...
		protected void updateItem(PageSearchResult item, boolean empty) {
			...
			if (item != null && !empty) {
				...
				final String text = item.getItems().stream()
						.filter(searchResult -> searchResult.getTextSnippet() != null).map(SearchResult::getTextSnippet)
